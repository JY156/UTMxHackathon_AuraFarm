    ...
    useFarmStore.setState((state) => {
      ...
      const newAlerts = hasAlert ? state.alerts : [
        ...state.alerts.filter(a => a.id !== alertId),
        {
          ...
          message: 'CRITICAL VISION ALERT: Leaf Rust detected on Rack 3! Spore spread risk. Please inspect all nearby lettuce cups and throw away the affected ones immediately.',
          actionRequired: true,
          resolved: false,
          timestamp: Date.now(),
          ...
          rackId: 3,
          shelf: 0,
          imageUrl: '/images/leaf_rust_cv.png'
        }
      ]
      return {
        sensors: state.sensors ? { ...state.sensors, humidity: 45.0 } : null,
        actuators: state.actuators ? { ...state.actuators, fan: true, mist: true } : null,
        cvData: state.cvData ? {
          ...state.cvData,
          diseases_detected: [{ name: "Leaf Rust", confidence: 0.94, severity: "critical" }]
        } : null,
        alerts: newAlerts,
        automationLog: [
          ...state.automationLog,
          `🌿 [SYSTEM] Biological threat alert registered. Dosing anti-pathogen mist cycle...`,
          `💨 [AUTOMATION] Fan opened, mist nozzle spray activated. Target humidity lowered to 45.0% for spore inhibition.`
        ].slice(-5),
        toasts: [
          ...state.toasts,
          { id: Math.random().toString(), message: "Critical CV Alert: Leaf Rust detected on Rack 3!", type: 'error' },
          { id: Math.random().toString(), message: "Lowering humidity & running anti-pathogen mist cycle", type: 'info' }
        ]
      }
    })
  }
